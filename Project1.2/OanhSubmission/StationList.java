...
public class StationList
{
    ...
    public double simulateOneStep(){                      //simulate one step for all stations
        double sum = 0;                                   //total wait time of cars that are just fully charged
        int ct = 0;                                       //number of cars are just fully charged
        for (int i = 0; i < stationList.size(); i++){
        ...
        if (ct >0) return sum/ct;                         // average waiting time of cars that are just fully charged
        return 0;
    }
    
    public String commandE(){                               //Perform request when user presses "E" 
        String message = "";
        for (int i = 0; i < stationList.size(); i++){        //Print ids of cars at each station
