...
public class CarListTest
{
    ...
    @Test 
    public void testAdd(){
        RandomGaussian r = new RandomGaussian(100.0,5.0,4328472);                          //create stations and carList
        StationList stations = new StationList(r); 
        CarList cars = new CarList(stations);
        for (int i = 0; i < 3; i++){                                                       //add cars to stations
    ...
    public void testRemoveFrontCar(){
        ...
        CarList cars = new CarList(stations);
        for (int i = 0; i < 3; i++){
            cars.add(new Car(i,2.0,new Battery(2.0,4.0,2.0,10.0),stations));            //add cars to stations
        }
        cars.removeFrontCar();                                                          //remove front car
